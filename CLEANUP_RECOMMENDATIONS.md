...
### 2. Typed-DTO mapping layer for UDTableSvc (replaces UDXSvc)
...
This entry covers the typed-DTO feature plus a small set of related changes — service rename, raw-API method rename, and an internal fix to `GetByIDAsync` — that the typed-DTO design surfaced as worth doing in the same release cycle. All non-breaking in practice (no current consumer uses `UDXSvc` for production data).

**Proposed shape:** add a parallel API on the (renamed) service that takes user-defined DTO classes with attribute-decorated properties, and handles the UD-column mapping internally.

```csharp
// User defines once, in their own project:
public class OrderTracking
{
    [UDTableColumn("Key1")]         public string Category { get; set; }  // "ORDER_TRACKING"
    [UDTableColumn("Key2")]         public string OrderNum { get; set; }
    [UDTableColumn("ShortChar01")]  public string CustomerName { get; set; }
    [UDTableColumn("ShortChar20")]  public string SearchTags { get; set; }  // "expedited,acme-corp"
...
using (var udTable = new UDTableSvc("pilot"))
{
    await udTable.SaveAsync("UD22", new OrderTracking { ... });
    var loaded = await udTable.GetByIDAsync<OrderTracking>("UD22", key1: "ORDER_TRACKING", key2: "12345");
}
```

#### Design decisions

- **Approach: attribute-based explicit mapping.** Considered convention-based auto-mapping (property order → column number) and a fluent-builder registration pattern. Attributes win because the mapping is self-documenting on the type, survives property renames, and avoids silent column-shift bugs when a property is added to the middle of a class. The `[UDTableColumn("...")]` declaration is the persistent contract between the DTO and the UD-table data.
...
- **Strict exceptions on rule violations.** At first use of a DTO type, throw `InvalidOperationException` for: invalid column name (e.g. `[UDTableColumn("Whatever05")]`), type mismatch (e.g. `Date01` on a `string` property, or a non-numeric type on a `Number##` column), or duplicate column mapping (two properties pointing at the same column). At save time, throw `UDTableColumnCapacityException` (a new type) when a `string` value exceeds the column's capacity (100 for `ShortChar`, 1000 for `Character`, ~50 for `Key`), with a message naming the property, column, value length, and capacity. Loud, early failure beats silent data corruption.
...
#### Relationship to existing legend helpers

UDTableSvc (the renamed UDXSvc — see "Service / API renames" below) has three legend-related members that all remain after this feature lands:

- **`ParseColumnLegend(string character10)`** — public static helper that parses a Character10 legend string into `Dictionary<string, string>`. Stays public. Useful for raw-`UDRow` users who want to inspect legend metadata, and for the typed-DTO mapper if/when it adds optional read-time legend validation.
- **`BuildColumnLegend(IDictionary<string, string> legend)`** — public static helper that builds a legend string from a map. Stays public. The typed-DTO implementation calls this internally to construct the auto-emitted legend, unifying both APIs around the same builder.
- **`UDRow.ToMappedValues()`** — instance method that re-keys a row's column values by their legend's meanings, returning `Dictionary<string, string>`. Stays public. Useful for raw-`UDRow` users who want domain-named access to values without writing a DTO class, and for callers reading legacy rows whose schema isn't known at compile time. Its docstring should gain a short note steering known-schema callers toward the typed-DTO API instead: "If your row's schema is known at compile time, the typed-DTO API on `UDTableSvc` gives the same access via typed properties, without the dictionary indirection."

Nothing about the legend infrastructure becomes obsolete. The typed-DTO API is a *convenience layer* over the same primitives, not a replacement.

#### Service / API renames coordinated with this feature

Three changes to the raw API are bundled into the same release because the typed-DTO design surfaced them as inconsistencies:

- **`UDXSvc` → `UDTableSvc`.** The `UDX*` prefix is a Keri-invented placeholder where the `X` does no work and reads ambiguously as "UD codes" to anyone familiar with Epicor's UD landscape (`UD codes` are a separate concept from `UD tables`, handled by `UserCodesSvc`). Renaming to `UDTableSvc` is self-explanatory and consistent with how other Keri services name themselves (`PartSvc`, `CustomerSvc`, `JobEntrySvc` all describe what they wrap). The exposing property on `EpicorClient` becomes `client.UDTable`. The rename cascades to the new types in this feature: `UDTableColumnAttribute`, `UDTableColumnCapacityException`, `UDTableMapping<T>`. This is a breaking change in the technical sense; the actual cost is zero (no consumer is using `UDXSvc` for production data yet).

- **`UDXSvc.GetAllAsync` → `UDTableSvc.QueryAsync`.** `GetAllAsync` is a Keri-invented name — it doesn't wrap a specific Epicor BO action (it's an OData entity-set read against `/Ice.BO.{UDTable}Svc/{UDTable}s` with optional filtering). The "GetAll" name reads as "fetch every row, no filter," which is misleading because the method supports filtering. `QueryAsync` is more honest: it's an OData query that may filter, that may select columns, that may limit results. Same body, better name.

- **`UDXSvc.GetByIDAsync` — fix the guts, keep the name.** The current `GetByIDAsync` is misnamed in a subtle way: it claims an Epicor-naming-convention name (matching `Ice.BO.{UDTable}Svc.GetByID`) but its body actually does an OData filtered read against the entity set, joining all five keys with `and` in a `$filter` clause. Epicor's real `GetByID` action is a direct BO action call: `/Ice.BO.{UDTable}Svc/GetByID?key1=...&key2=...&key3=...&key4=...&key5=...` — same shape as `PartSvc.GetByIDAsync` calls `Erp.BO.PartSvc/GetByID?partNum=...`. Fix the implementation to call the real action (GET with query parameters, single round trip), preserving the public signature so callers see no change. After this fix, `GetByIDAsync` actually does what its name promises, and the "filtered query by partial keys" use case is properly served by the renamed `QueryAsync`.

These three changes are coherent: the service name, the OData-read method name, and the BO-action method body all become honest about what they are.

#### Precursor cleanup (UDRow.cs edits, separate small commit)
...
| # | Commit | Description |
|---|---|---|
| 1 | UDRow.cs precursor | Strip `Key1 = "ROW_INDICATOR"` default; rewrite Key1 doc; drop Number20 "checksum" reservation doc. Single file. |
| 2 | UDXSvc → UDTableSvc rename | Mechanical rename across codebase. File: `UDXSvc.cs` → `UDTableSvc.cs`. Class, property on `EpicorClient`, README mentions, tests, demo/POCs, EXAMPLES references. Pure rename, no logic changes. |
| 3 | UDTableSvc raw-API rework | Rename `GetAllAsync` → `QueryAsync`. Rewrite `GetByIDAsync` body to call Epicor's real `GetByID` BO action (GET with key1–5 query parameters, same shape as `PartSvc.GetByIDAsync`). Update tests + any callers in demo/POCs. Add note to `UDRow.ToMappedValues()` docstring steering known-schema callers toward the typed-DTO API. |
| 4 | UDTableColumnAttribute + UDTableColumnCapacityException | Two new small files in `EpicorSvcs/Platform/`. |
| 5 | UDTableMapping\<T\> + tests | Internal reflection-based mapper class. Validation rules (invalid column, type mismatch, duplicate, capacity). Numeric type conversion logic. Auto-emit legend logic for unmapped Character10. Reflection cache. Tests in isolation before UDTableSvc is touched. |
| 6 | Typed-DTO methods on UDTableSvc + tests | `SaveAsync<T>`, `GetByIDAsync<T>`, `QueryAsync<T>` — each a thin wrapper over the corresponding raw `UDRow`-based method plus a mapping step. Tests covering the integration of mapper into the CRUD flow. |
| 7 | Docs + changelog | New "Typed UD-table access" section in `README.md`. Worked examples in `EXAMPLES_EPICOR.md` illustrating the reserved-column conventions through 3–4 progressively complete DTOs (minimal example showing automation; example mapping Key1 showing category convention; example mapping ShortChar20 showing search tags; example mapping CheckBox20 showing soft-delete usage). No "starter DTO" base class — conventions are taught through documentation and examples, not through a class hierarchy. CHANGELOG `[0.2.0] — Unreleased` block listing all changes: typed-DTO API additions, `UDXSvc → UDTableSvc` rename, `GetAllAsync → QueryAsync` rename, `GetByIDAsync` body fix. |

#### Estimated effort

8–12 hours of focused work end to end.

#### Open question deferred to implementation time

Support `UDTableColumn(UDTableCol.Character01)` enum-based attribute as a typo-safe alternative to the string version. Both shapes can coexist (two attribute overloads, internal logic normalizes). Decide at implementation; default is string-only for v1.

This is a real feature with coordinated raw-API improvements. It deserves its own design pass, its own feature branch, and its own CHANGELOG block.
